...
import { signupUser, signupUserParams } from "@/axios/(user)/signupUser";
import { verifyUser, VerifyUserParams } from "@/axios/(user)/verifyUser";
...
export default function Page() {
  ...
  const signupMutation = useMutation({
    mutationFn: ({ name, email, password }: signupUserParams) =>
  ...
  const verifyOtpMutation = useMutation({
    mutationFn: ({ userData, otp }: VerifyUserParams) =>
      verifyUser({ userData, otp }),
    onSuccess: () => {
  ...
  return (
    <div className="w-full py-10 min-h-[85vh] bg-[#f1f1f1] text-black">
